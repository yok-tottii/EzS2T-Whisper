...
EzS2T-Whisperは、2つの独立したUIを持ち、それぞれが異なる役割を果たします：

#### **システムトレイ（メニューバー）**
常駐型の軽量インターフェース。リアルタイムな操作と状態表示に特化。

**機能:**
- 📊 **状態表示**: アイコンの色で録音中（オレンジ）/処理中（緑）/待機中（グレー）を表示
- 🔧 **設定を開く**: ブラウザで詳細設定画面を起動
- 🎤 **録音テスト**: 5秒間の録音→文字起こし→通知のテスト実行
- 🚪 **終了**: アプリケーションを終了

#### **Web設定画面**
ブラウザベースの詳細設定インターフェース。すべての設定項目を管理。

**アクセス方法:**
- メニューバーのアイコンから「設定を開く...」を選択

**設定項目:**
- **システム権限**: マイク・アクセシビリティの状態確認と設定画面への誘導
- **ホットキー設定**: 録音開始のキーコンビネーションをカスタマイズ（競合検出機能付き）
- **録音モード**: 押下中録音 / トグル切替を選択
- **モデル選択**: 推奨モデルまたはカスタムモデルを選択（ファイルピッカー付き）
